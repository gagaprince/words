    ...
    async parseLesson(data){
        ...
        for(let i=0;i<frontendlist.length;i++){
            ...
            for(let j=0;j<frontend.length;j++){
                ...
                if(wordObj){
                    const [word,word_zn,yinbiao,ks,un,voice,pic,sentence,cixing,video,captions,multi_mean] = wordObj;
                    console.log(word,word_zn,yinbiao,voice,pic,sentence,cixing,video,captions,multi_mean);
                    await db.saveIntoWordsWhenNotIn({
                        word,word_zn,yinbiao,voice,pic,sentence,cixing,video,
